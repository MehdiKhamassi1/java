...
public class docteurservice implements IService<docteur> {
    ...
    public void create(docteur docteur) throws SQLException {
        String sql = "INSERT INTO user (email, roles, password, nom, prenom, numtel, birth, profile_image, gender, specialite, user_type) VALUES ('"
                + docteur.getEmail() + "', '"
                + docteur.getRoles() + "', '"
                + docteur.getPassword() + "', '"
                + docteur.getNom() + "', '"
    ...
    public void update(docteur docteur) throws SQLException {
        String sql = "UPDATE user SET email = ?, roles = ?, password = ?, nom = ?, prenom = ?, numtel = ?, birth = ?, profile_image = ?, gender = ?, specialite = ? WHERE id = ?";
    ...
    public List<docteur> read() throws SQLException {
        ...
        while (rs.next()) {
            ...
            docteur.setProfileImage(rs.getString("profile_image"));
