...
public class rendezvouzService implements IService<rendezvouz> {
    ...
    public void ajout(rendezvouz rendezvouz,int idd) throws SQLException {
        String sql = "INSERT INTO rendezvouz (daterdv, email, local_id,user_id) VALUES (?, ?, ?,?)";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setTimestamp(1, rendezvouz.getDaterdv());
            ps.setString(2, rendezvouz.getEmail());
            ps.setInt(3, rendezvouz.getLocal_id());
    ...
    public List<rendezvouz> specc(int id) throws SQLException {
        String sql = "select * from rendezvouz where user_id= ?";
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setInt(1, id);
        ResultSet rs = statement.executeQuery();
        List<rendezvouz> people = new ArrayList<>();
