...
public class publicationservice implements IService<publication>{
    ...
    public List<publication> zert(int id) throws SQLException {
        String sql = "select * from publication where user_id= ?";
        PreparedStatement statement = connection.prepareStatement(sql);
    ...
    public void createe(publication publication,int idd) throws SQLException {
        String sql = "INSERT INTO publication (titre, contenu, image, user_id) VALUES (?, ?, ?, ?)";
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setString(1, publication.getTitre());
        statement.setString(2, publication.getContenu());
        statement.setString(3, publication.getImage());
        statement.setInt(4, idd);
        statement.executeUpdate();
    }
