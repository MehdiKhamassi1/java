...
public class patientservice implements IService<patient>{
    ...
    public void create(patient patient) throws SQLException {
        String sql = "INSERT INTO user (email, roles, password, nom, prenom, numtel, birth, profile_image, gender, user_type) VALUES ('"
                + patient.getEmail() + "', '"
                + patient.getRoles() + "', '"
                + patient.getPassword() + "', '"
                + patient.getNom() + "', '"
    ...
    public void update(patient user) throws SQLException {
        String sql = "UPDATE user SET email = ?, roles = ?, password = ?, nom = ?, prenom = ?, numtel = ?, birth = ?, profile_image = ?, gender = ? WHERE id = ?";
    ...
    public List<patient> read() throws SQLException {
        ...
        while (rs.next()) {
            ...
            pat.setProfileImage(rs.getString("profile_image"));
